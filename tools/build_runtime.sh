...
OUT="$ROOT/MiSTer_Audit.sh"
TMP="${TMPDIR:-/tmp}/mister-audit-build.$$"
trap 'rm -f "$TMP"' EXIT
...
if [[ "${1:-}" == "--check" ]]; then
  ...
  [[ "$dispatch_count" -eq 1 ]] || {
    echo "ERROR: expected exactly one final runtime dispatch block, found $dispatch_count"
    exit 1
  }

  for fn in run_audit run_update_tools main_menu; do
    count="$(grep -Fxc "${fn}() {" "$TMP" || true)"
    [[ "$count" -eq 1 ]] || {
      echo "ERROR: expected exactly one ${fn} definition, found $count"
      exit 1
    }
  done
  ...
  [[ "$last_nonblank" == "esac" ]] || {
    echo "ERROR: executable or stray content exists after final dispatch"
    exit 1
  }
