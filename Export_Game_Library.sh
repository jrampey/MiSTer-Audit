...
if [ "$USE_HASH_CACHE" -eq 0 ]; then : > "$WORK.hashjobs"; : > "$PREHASH_RESULTS"; while IFS=$'\t' read -r psystem pp pfile pext prest; do should_hash "$psystem" "$pext" && printf '%s\0' "$pp" >> "$WORK.hashjobs"; done < "$PLAN"; pv_start=$SECONDS; if [ -s "$WORK.hashjobs" ]; then XARGS_PARALLEL_ARGS=""; if xargs --help 2>&1 | grep -q -- '-P'; then XARGS_PARALLEL_ARGS="-P $FULL_VERIFY_WORKERS"; fi; if command -v sha1sum >/dev/null 2>&1; then xargs -0 -n1 $XARGS_PARALLEL_ARGS sh -c 'p="$1"; h=$(sha1sum "$p" 2>/dev/null); h=${h%% *}; printf "%s\t%s\n" "$h" "$p"' sh < "$WORK.hashjobs" > "$PREHASH_RESULTS"; else xargs -0 -n1 $XARGS_PARALLEL_ARGS sh -c 'p="$1"; h=$(openssl sha1 "$p" 2>/dev/null); h=${h##* }; printf "%s\t%s\n" "$h" "$p"' sh < "$WORK.hashjobs" > "$PREHASH_RESULTS"; fi; while IFS=$'\t' read -r ph pp; do [ -n "$pp" ] && PREHASH_SHA_BY_PATH["$pp"]="$ph"; done < "$PREHASH_RESULTS"; fi; FULL_VERIFY_PARALLEL_SECONDS=$((SECONDS-pv_start)); fi
REPORT_START=$(date +%s)
...
while IFS=$'\t' read -r -u 3 system p file ext stem clean region kind; do
  ...
  if [ -n "$sha1" ] && [ "$sha1" != "UNAVAILABLE" ]; then
    ...
    if [ -n "${DAT_NAME_BY_SHA[$hkey]+x}" ]; then
      ...
      if [ -n "$dat_rom" ]; then canonical_file="${dat_rom##*/}"; canonical_stem="${canonical_file%.*}"; [ -n "$canonical_stem" ] && clean="$(clean_title "$canonical_stem")"; [ -n "$meta_region" ] && region="$meta_region"; [ -n "$meta_release" ] && kind="$meta_release"; proposed="$canonical_file"; elif [ -n "$dat_name" ]; then clean="$(clean_title "$dat_name")"; [ -n "$meta_region" ] && region="$meta_region"; [ -n "$meta_release" ] && kind="$meta_release"; proposed="$clean$(suffix_for "$region" "$kind").$ext"; fi
