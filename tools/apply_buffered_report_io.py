...
old='csv_row() { local dest="$1" out="" v; shift; for v in "$@"; do v="${v//\"/\"\"}"; [ -n "$out" ] && out+=","; out+="\"$v\""; done; printf \'%s\\n\' "$out" >> "$dest"; }'
new='csv_row() { local dest="$1" out="" v fd=""; shift; for v in "$@"; do v="${v//\"/\"\"}"; [ -n "$out" ] && out+=","; out+="\"$v\""; done; case "$dest" in "$STAGE_CSV") fd=10 ;; "$STAGE_REN") fd=11 ;; "$STAGE_SAVE_REN") fd=12 ;; "$STAGE_DAT_MATCH") fd=14 ;; "$STAGE_DAT_UNMATCHED") fd=15 ;; "$STAGE_LOCATION_AUDIT") fd=16 ;; esac; if [ -n "$fd" ]; then printf \'%s\\n\' "$out" >&"$fd"; else printf \'%s\\n\' "$out" >> "$dest"; fi; }'
assert old in s; s=s.replace(old,new,1)
marker=': > "$HASH_ROWS"; : > "$COLLISION_ROWS"\nTOTAL=0;'
assert marker in s
s=s.replace(marker,': > "$HASH_ROWS"; : > "$COLLISION_ROWS"\n# Keep hot report files open during the per-ROM loop to avoid repeated FAT open/close I/O.\nexec 9>>"$STAGE_OUT" 10>>"$STAGE_CSV" 11>>"$STAGE_REN" 12>>"$STAGE_SAVE_REN" 14>>"$STAGE_DAT_MATCH" 15>>"$STAGE_DAT_UNMATCHED" 16>>"$STAGE_LOCATION_AUDIT" 17>>"$HASH_ROWS" 20>>"$HASH_CACHE_NEW" 21>>"$COLLISION_ROWS"\nTOTAL=0;',1)
