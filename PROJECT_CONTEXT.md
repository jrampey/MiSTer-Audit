...
MiSTer ROM Library Auditor audits a complete `/media/fat/games` MiSTer FPGA ROM library. It uses hash/DAT identification, MiSTer-aware system metadata, canonical No-Intro naming, region/release classification, save pairing, duplicate detection, location auditing, safe rename proposals, full-library verification, caching infrastructure, and consolidated `MiSTer_Library_Audit.txt` reports.
...
Full Verification is the standard and only audit mode. Every run rescans the complete library and recalculates supported hashes before rebuilding global safety/accounting state and reports.
...
The former Fast Audit mode and audit-mode selector are retired. Full Verification is always used.

- Audit performance telemetry reports stage timings for the standard Full Verification path.
...
Keep the MiSTer-facing distribution as one `MiSTer_Audit.sh`, but maintain modular source under `src/` and regenerate it with `tools/build_runtime.sh`. Full Verification is the single audit path.
