...
REVIEW_QUEUE="$AUDIT/needs_review.csv"
SUPPORT_REPORT="$AUDIT/support_files.csv"
RELEASE_FAMILIES="$AUDIT/release_families.csv"
DISC_REPORT="$AUDIT/disc_media.csv"
...
STAGE_REVIEW_QUEUE="$STAGE_DIR/needs_review.csv"
STAGE_SUPPORT_REPORT="$STAGE_DIR/support_files.csv"
STAGE_RELEASE_FAMILIES="$STAGE_DIR/release_families.csv"
STAGE_DISC_REPORT="$STAGE_DIR/disc_media.csv"
...
cleanup() { rm -f "$GAME_LIST" "$SAVE_LIST" "$SAVE_INDEX" "$PLAN" "$DAT_INDEX" "$HASH_ROWS" "$COLLISION_ROWS" "$HASH_CACHE_NEW" "$CLASS_CACHE_NEW" "$DISCOVERY_SNAPSHOT_NEW" "$DISCOVERY_CHANGED" "$PREHASH_RESULTS" "$WORK.duphashes" "$WORK.hashjobs" "$WORK.release_rows"; rm -rf "$STAGE_DIR"; }
...
if [ -s "$WORK.release_rows" ]; then
  awk -F '\t' 'BEGIN{OFS="\t"} {k=$1 SUBSEP $2; count[k]++; if($3=="Retail/Standard") retail[k]++; else special[k]++; cats[k SUBSEP $3]=1; sys[k]=$1; title[k]=$2} END {for(k in count){c=""; prefix=k SUBSEP; for(x in cats) if(index(x,prefix)==1){v=substr(x,length(prefix)+1); c=(c?c";":"") v} print sys[k],title[k],count[k],retail[k]+0,special[k]+0,c}}' "$WORK.release_rows" | LC_ALL=C sort > "$WORK.release_family_summary"
  while IFS="$(printf '\t')" read -r rf_sys rf_title rf_count rf_retail rf_special rf_cats; do
    csv_row "$STAGE_RELEASE_FAMILIES" "$rf_sys" "$rf_title" "$rf_count" "$rf_retail" "$rf_special" "$rf_cats"
  done < "$WORK.release_family_summary"
  rm -f "$WORK.release_family_summary"
fi
