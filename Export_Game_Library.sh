...
build_completion_reference() {
  ...
  for h in "${!DAT_NAME_BY_SHA[@]}"; do
    ...
    if [ -z "${COMPLETION_REFERENCE_KEYS[$key]+x}" ]; then
      COMPLETION_REFERENCE_KEYS["$key"]=1; COMPLETION_TOTAL_BY_SYSTEM["$sys"]=$(( ${COMPLETION_TOTAL_BY_SYSTEM["$sys"]:-0} + 1 ))
...
record_completion_owned() {
  ...
  [ -n "$sys" ] && [ -n "$title" ] || return 0; completion_record_eligible "$region" "$release" "$license" || return 0; key="$sys|$title"
...
load_hash_cache() {
  ...
  while IFS=$'\t' read -r p sig sha ds dn dr dsrc; do
    [ "$p" = "path" ] && continue; k="$p|$sig"; CACHE_SHA["$k"]="$sha"; CACHE_ENTRIES_LOADED=$((CACHE_ENTRIES_LOADED+1))
    if [ "$old_db" = "$HASH_DB_FINGERPRINT" ]; then CACHE_DAT_STATUS["$k"]="$ds"; CACHE_DAT_NAME["$k"]="$dn"; CACHE_DAT_ROM["$k"]="$dr"; CACHE_DAT_SOURCE["$k"]="$dsrc"; fi
  done < "$HASH_CACHE"
}
...
is_support_file() {
  ...
  case "$path" in */bios/*|*/bioses/*|*/firmware/*|*/kickstart/*|*/bootrom/*|*/boot_rom/*|*/boot-rom/*|*/system_rom/*|*/system-rom/*|*/machine_rom/*|*/machine-rom/*|*/testrom/*|*/test_rom/*|*/test-rom/*|*/diagnostic/*|*/diagnostics/*|*/utilities/*|*/utility/*) return 0 ;; esac
  case "$file" in bios.*|boot.rom|boot.bin|boot[0-9]*.rom|boot[0-9]*_*.rom|boot[0-9]*-*.rom|firmware.*|kickstart.rom|cd_bios.rom|uni-bioscd.rom|kanji.rom|empty.rom) return 0 ;; esac
...
clean_title() {
  ...
  s="$(trim "$s")"; while [[ "$s" == *"  "* ]]; do s="${s//  / }"; done; s="${s% -}"; s="${s% _}"; s="$(trim "$s")"; [ -z "$s" ] && s="$1"; printf '%s' "$s"
}
...
location_status() {
  local current="${1,,}" expected="${2,,}"; [ -z "$expected" ] && { echo "Unknown"; return; }; [ "$current" = "$expected" ] && { echo "OK"; return; }
  case "$expected" in nes) case "$current" in nes|fds|famicom) echo "OK (compatible folder)"; return;; esac ;; gameboy) case "$current" in gameboy|game\ boy|gb) echo "OK (compatible folder)"; return;; esac ;; n64) case "$current" in n64|nintendo64|nintendo\ 64) echo "OK (compatible folder)"; return;; esac ;; esac
  echo "MISFILED"
}

# Static stage output plus a progress heartbeat every 30 seconds. Avoid a
# background spinner because carriage-return animation collides with MiSTer UI output.
START_TIME=$(date +%s)
PROGRESS_START_SECONDS=$SECONDS
...
progress_check() {
  ...
  now=$SECONDS
  if [ $((now - LAST_PROGRESS_SECONDS)) -ge 30 ]; then
    elapsed=$((now - PROGRESS_START_SECONDS)); pct=0
    ...
    if [ "$stage" = "Building reports" ]; then printf ' | DAT matches: %s | Unmatched: %s' "${DAT_MATCHED:-0}" "$(( ${HASHED:-0} - ${DAT_MATCHED:-0} ))"; fi
...
if [ -f "$HASH_DB_TSV" ]; then
  IFS=$'\t' read -r dbh _ < "$HASH_DB_TSV"; [ "$dbh" = "sha1" ] || { SELF_CHECK_STATUS="FAIL"; SELF_CHECK_NOTES="$SELF_CHECK_NOTES invalid:hash-db-header"; }
  DB_HEADER=$(head -n 1 "$HASH_DB_TSV" 2>/dev/null); REQUIRED_DB_HEADER=$'sha1\tcanonical_title\tcanonical_rom_name\tdat_source\tsize\tcrc32\tmd5\tmister_system\tmister_core\texpected_folder\tregion\trelease_type\tlicense_status'
  if [ "$DB_HEADER" = "$REQUIRED_DB_HEADER" ]; then METADATA_LAYER_STATUS="MiSTer-aware"; else METADATA_LAYER_STATUS="INVALID"; SELF_CHECK_STATUS="FAIL"; SELF_CHECK_NOTES="$SELF_CHECK_NOTES invalid:mister-aware-db-schema"; fi
  DB_LINE_COUNT=$(wc -l < "$HASH_DB_TSV" 2>/dev/null); DB_LINE_COUNT=${DB_LINE_COUNT//[[:space:]]/}; [ "${DB_LINE_COUNT:-0}" -ge 1000 ] 2>/dev/null || { SELF_CHECK_STATUS="FAIL"; SELF_CHECK_NOTES="$SELF_CHECK_NOTES suspiciously-small:hash-db"; }
fi
...
render_audit_menu() {
  local fast_prefix="  " full_prefix="  "; [ "$AUDIT_MENU_SELECTION" -eq 1 ] && fast_prefix="> " || full_prefix="> "
  printf '%s\n' '+--------------------------------------------------+'
  printf '%s\n' '| AUDIT MODE                                       |'
  printf '%s\n' '+--------------------------------------------------+'
  ...
  printf '%s\n' '     Recalculate every supported SHA-1.'
  printf '%s\n' '----------------------------------------------------'
  printf '%s\n' '1/2 or arrows select | Enter = Fast | Auto = 15s'
}
render_audit_menu
while :; do
  AUDIT_KEY=""
  if ! IFS= read -rsn1 -t 15 AUDIT_KEY; then AUDIT_MENU_SELECTION=1; break; fi
  case "$AUDIT_KEY" in
    "") AUDIT_MENU_SELECTION=1; break ;; 1) AUDIT_MENU_SELECTION=1; break ;; 2) AUDIT_MENU_SELECTION=2; break ;;
    $'\x1b') IFS= read -rsn1 -t 0.15 AUDIT_KEY2 || AUDIT_KEY2=""; if [ "$AUDIT_KEY2" = "[" ]; then IFS= read -rsn1 -t 0.15 AUDIT_KEY3 || AUDIT_KEY3=""; case "$AUDIT_KEY3" in A) AUDIT_MENU_SELECTION=1; break ;; B) AUDIT_MENU_SELECTION=2; break ;; esac; fi ;;
  esac
done
...
echo; echo "Selected: $AUDIT_MODE"; printf '%s\n\n' '----------------------------------------------------'
...
if [ -d "$SAVES" ]; then
  ...
  while IFS= read -r sp; do [ -z "$sp" ] && continue; sf="${sp##*/}"; sstem="${sf%.*}"; save_key_idx="${sstem,,}"; if [ -n "${SAVES_BY_STEM[$save_key_idx]:-}" ]; then SAVES_BY_STEM["$save_key_idx"]+=$'\n'"$sp"; else SAVES_BY_STEM["$save_key_idx"]="$sp"; fi; printf '%s\t%s\n' "$save_key_idx" "$sp" >> "$SAVE_INDEX"; SAVE_PROCESSED=$((SAVE_PROCESSED+1)); progress_check "Indexing saves" "$SAVE_PROCESSED" "$SAVE_SCAN_COUNT"; done < "$SAVE_LIST"
fi
SAVE_END=$(date +%s); DB_START=$SAVE_END
echo "[3/5] Loading hash database..."; build_dat_index; echo "    Hash database source: $HASH_DB_SOURCE"; echo "    Hash records indexed: $HASH_INDEX_COUNT"; build_completion_reference; load_hash_cache; DB_END=$(date +%s); CLASSIFY_START=$DB_END

echo "[4/5] Classifying titles and collisions..."; : > "$PLAN"; declare -A NAME_COUNTS; SKIPPED=0; CLASSIFIED=0
while IFS= read -r p; do
  [ -z "$p" ] && continue; rel="${p#$GAMES/}"; system="${rel%%/*}"; [ "$system" = "$rel" ] && system="Unknown"; file="${p##*/}"; ext="${file##*.}"; stem="${file%.*}"
  case "${ext,,}" in md|gen) system="MegaDrive" ;; 32x) system="S32X" ;; esac
  if is_support_file "$p" "$file"; then SKIPPED=$((SKIPPED+1)); continue; fi
  region="$(region_of "$stem")"; kind="$(kind_of "$stem")"; clean="$(clean_title "$stem")"; suffix="$(suffix_for "$region" "$kind")"; proposed="$clean$suffix.$ext"; key="${system,,}|${proposed,,}"; NAME_COUNTS["$key"]=$(( ${NAME_COUNTS["$key"]:-0} + 1 ))
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$system" "$p" "$file" "$ext" "$stem" "$clean" "$region" "$kind" >> "$PLAN"; CLASSIFIED=$((CLASSIFIED+1)); progress_check "Classifying titles" "$CLASSIFIED" "$GAME_SCAN_COUNT"
done < "$GAME_LIST"
CLASSIFY_END=$(date +%s)
...
if [ "$USE_HASH_CACHE" -eq 0 ]; then
  ...
  while IFS=$'\t' read -r psystem pp pfile pext prest; do should_hash "$psystem" "$pext" && printf '%s\0' "$pp" >> "$WORK.hashjobs"; done < "$PLAN"
  pv_start=$SECONDS
  if [ -s "$WORK.hashjobs" ]; then
    XARGS_PARALLEL_ARGS=""; if xargs --help 2>&1 | grep -q -- '-P'; then XARGS_PARALLEL_ARGS="-P $FULL_VERIFY_WORKERS"; fi
    if command -v sha1sum >/dev/null 2>&1; then xargs -0 -n1 $XARGS_PARALLEL_ARGS sh -c 'p="$1"; h=$(sha1sum "$p" 2>/dev/null); h=${h%% *}; printf "%s\t%s\n" "$h" "$p"' sh < "$WORK.hashjobs" > "$PREHASH_RESULTS"; else xargs -0 -n1 $XARGS_PARALLEL_ARGS sh -c 'p="$1"; h=$(openssl sha1 "$p" 2>/dev/null); h=${h##* }; printf "%s\t%s\n" "$h" "$p"' sh < "$WORK.hashjobs" > "$PREHASH_RESULTS"; fi
...
while IFS=$'\t' read -r system p file ext stem clean region kind; do
  [ -z "$p" ] && continue; suffix="$(suffix_for "$region" "$kind")"; base="$clean$suffix"; proposed="$base.$ext"; key="${system,,}|${proposed,,}"; collision="None"
  if [ "${NAME_COUNTS["$key"]:-0}" -gt 1 ]; then n=$(( ${SEEN_NAMES["$key"]:-0} + 1 )); SEEN_NAMES["$key"]=$n; proposed="$base [Variant $n].$ext"; collision="Resolved variant $n of ${NAME_COUNTS["$key"]}"; COLLISIONS=$((COLLISIONS+1)); fi
  ...
  if should_hash "$system" "$ext"; then
    HASH_ELIGIBLE=$((HASH_ELIGIBLE+1)); SYSTEM_ELIGIBLE["$system"]=$(( ${SYSTEM_ELIGIBLE["$system"]:-0} + 1 )); sig="$(file_signature "$p")"; cache_key="$p|$sig"; cached_sha=""; if [ "$USE_HASH_CACHE" -eq 1 ]; then cached_sha="${CACHE_SHA[$cache_key]:-}"; fi
    if [ -n "$cached_sha" ]; then sha1="$cached_sha"; HASH_REUSED=$((HASH_REUSED+1)); if [ -n "${CACHE_DAT_STATUS[$cache_key]+x}" ]; then dat_status="${CACHE_DAT_STATUS[$cache_key]}"; dat_name="${CACHE_DAT_NAME[$cache_key]:-}"; dat_rom="${CACHE_DAT_ROM[$cache_key]:-}"; dat_source="${CACHE_DAT_SOURCE[$cache_key]:-}"; else dat_status="No match"; fi
  ...
  if [ -n "$sha1" ] && [ "$sha1" != "UNAVAILABLE" ]; then
    HASHED=$((HASHED+1)); printf '%s\t%s\t%s\t%s\t%s\n' "${sha1,,}" "$system" "$p" "$file" "$clean" >> "$HASH_ROWS"; hkey="${sha1,,}"; matched_hkey="$(normalized_dat_hash "$p" "$ext" "$hkey")"; if [ "$matched_hkey" != "$hkey" ]; then hkey="$matched_hkey"; sha1="$matched_hkey"; dat_status="Normalized SHA-1"; fi
    if [ -n "${DAT_NAME_BY_SHA[$hkey]+x}" ]; then
      ...
      if [ -n "$dat_rom" ]; then canonical_file="${dat_rom##*/}"; canonical_ext="${canonical_file##*.}"; canonical_stem="${canonical_file%.*}"; [ -n "$canonical_stem" ] && clean="$(clean_title "$canonical_stem")"; [ -n "$meta_region" ] && region="$meta_region"; [ -n "$meta_release" ] && kind="$meta_release"; proposed="$canonical_file"
      elif [ -n "$dat_name" ]; then clean="$(clean_title "$dat_name")"; [ -n "$meta_region" ] && region="$meta_region"; [ -n "$meta_release" ] && kind="$meta_release"; proposed="$clean$(suffix_for "$region" "$kind").$ext"; fi
      loc_status="$(location_status "$system" "$meta_folder")"; record_completion_owned "${meta_system:-$system}" "$dat_name" "$meta_region" "$meta_release" "$meta_license"; [ "$dat_status" = "Normalized SHA-1" ] || dat_status="Exact SHA-1"; DAT_MATCHED=$((DAT_MATCHED+1)); SYSTEM_MATCHED["$system"]=$(( ${SYSTEM_MATCHED["$system"]:-0} + 1 ))
    ...
    else SYSTEM_UNMATCHED["$system"]=$(( ${SYSTEM_UNMATCHED["$system"]:-0} + 1 )); csv_escape "$sha1" >> "$STAGE_DAT_UNMATCHED"; printf ',' >> "$STAGE_DAT_UNMATCHED"; csv_escape "$system" >> "$STAGE_DAT_UNMATCHED"; printf ',' >> "$STAGE_DAT_UNMATCHED"; csv_escape "$p" >> "$STAGE_DAT_UNMATCHED"; printf ',' >> "$STAGE_DAT_UNMATCHED"; csv_escape "$file" >> "$STAGE_DAT_UNMATCHED"; printf '\n' >> "$STAGE_DAT_UNMATCHED"; fi
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$p" "$sig" "${sha1,,}" "$dat_status" "$dat_name" "$dat_rom" "$dat_source" >> "$HASH_CACHE_NEW"
  fi
  final_key="${system,,}|${proposed,,}"; if [ -n "${FINAL_PROPOSAL_COUNTS[$final_key]+x}" ]; then FINAL_PROPOSAL_COUNTS["$final_key"]=$((FINAL_PROPOSAL_COUNTS["$final_key"]+1)); else FINAL_PROPOSAL_COUNTS["$final_key"]=1; fi
...
while IFS= read -r c_sys; do
  [ -n "$c_sys" ] || continue; c_total="${COMPLETION_TOTAL_BY_SYSTEM[$c_sys]:-0}"; c_owned="${COMPLETION_OWNED_BY_SYSTEM[$c_sys]:-0}"; c_missing=$((c_total-c_owned)); c_pct="$(awk -v a="$c_owned" -v b="$c_total" 'BEGIN{if(b>0) printf "%.2f", (a*100)/b; else printf "0.00"}')"
  csv_escape "$c_sys" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; csv_escape "$c_owned" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; csv_escape "$c_total" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; csv_escape "$c_missing" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; csv_escape "$c_pct" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; csv_escape "$COMPLETION_REGION_LABEL" >> "$STAGE_COMPLETION"; printf ',' >> "$STAGE_COMPLETION"; if [ "$COMPLETION_RETAIL_ONLY" -eq 1 ]; then csv_escape "Retail releases" >> "$STAGE_COMPLETION"; else csv_escape "All release types" >> "$STAGE_COMPLETION"; fi; printf '\n' >> "$STAGE_COMPLETION"
done < <(printf '%s\n' "${!COMPLETION_TOTAL_BY_SYSTEM[@]}" | LC_ALL=C sort)
for c_key in "${!COMPLETION_REFERENCE_KEYS[@]}"; do
  [ -n "${COMPLETION_OWNED_KEYS[$c_key]+x}" ] && continue; c_sys="${c_key%%|*}"; c_title="${c_key#*|}"; csv_escape "$c_sys" >> "$STAGE_MISSING_COMPLETION"; printf ',' >> "$STAGE_MISSING_COMPLETION"; csv_escape "$c_title" >> "$STAGE_MISSING_COMPLETION"; printf ',' >> "$STAGE_MISSING_COMPLETION"; csv_escape "${COMPLETION_TITLE_REGION[$c_key]:-}" >> "$STAGE_MISSING_COMPLETION"; printf ',' >> "$STAGE_MISSING_COMPLETION"; csv_escape "${COMPLETION_TITLE_RELEASE[$c_key]:-}" >> "$STAGE_MISSING_COMPLETION"; printf ',' >> "$STAGE_MISSING_COMPLETION"; csv_escape "${COMPLETION_TITLE_LICENSE[$c_key]:-}" >> "$STAGE_MISSING_COMPLETION"; printf '\n' >> "$STAGE_MISSING_COMPLETION"
done
...
{
  echo "MiSTer Game Library Audit Bundle v1.3"; echo "Generated: $(date)"; echo "READ-ONLY AUDIT REPORT - no ROM or save data is embedded."; echo "FULL LIBRARY REPORT - audit mode: $AUDIT_MODE."; echo "============================================================"; echo
  echo "[RUN SUMMARY]"; echo "Report scope: FULL LIBRARY"; echo "Audit mode: $AUDIT_MODE"; echo "Cache mode: $([ "$USE_HASH_CACHE" -eq 1 ] && echo "Incremental processing only" || echo "Bypassed for hash verification")"; echo "Files discovered: $GAME_SCAN_COUNT"; echo "Audit mode:             $AUDIT_MODE"; echo "Games/discs cataloged: $TOTAL"; echo "BIOS/support files skipped: $SKIPPED"; echo "Collision-affected rows: $COLLISIONS"; echo "Save matches: $SAVE_MATCHES"; echo "Files with SHA-1 available: $HASHED"; echo "Hashes reused from cache: $HASH_REUSED"; echo "Hashes calculated this run: $HASH_CALCULATED"; echo "Unsupported-format hashes skipped: $HASH_SKIPPED"; echo "Hash database source: $HASH_DB_SOURCE"; echo "MiSTer-aware metadata layer: $METADATA_LAYER_STATUS"; echo "Exporter build SHA-1: $EXPORTER_BUILD_SHA1"; echo "Audit integrity verdict: $AUDIT_VERDICT"; echo "Apply recommendation: $APPLY_RECOMMENDATION"; echo "Hash records indexed: $HASH_INDEX_COUNT"; echo "Exact DAT SHA-1 matches: $DAT_MATCHED"; echo
  echo "[AUDIT_METADATA]"; echo "schema_version=$AUDIT_SCHEMA_VERSION"; echo "exporter_version=1.3"; echo "build_sha1=$EXPORTER_BUILD_SHA1"; echo "audit_mode=$AUDIT_MODE"; echo "database_sha1=$HASH_DB_FINGERPRINT"; echo "metadata_layer=$METADATA_LAYER_STATUS"; echo "library_files=$GAME_SCAN_COUNT"; echo "cataloged_files=$TOTAL"; echo "self_check=$SELF_CHECK_STATUS"; echo "integrity_verdict=$AUDIT_VERDICT"; echo "apply_recommendation=$APPLY_RECOMMENDATION"; echo "integrity_notes=$INTEGRITY_NOTES"; echo
  echo "[DATABASE COVERAGE]"; echo "DAT-eligible ROMs: $HASH_ELIGIBLE"; echo "Matched: $DAT_MATCHED"; echo "Unmatched: $((HASHED-DAT_MATCHED))"; if [ "$HASHED" -gt 0 ]; then awk -v a="$DAT_MATCHED" -v b="$HASHED" 'BEGIN{printf "Match rate: %.2f%%\n", (a*100)/b}'; else echo "Match rate: 0.00%"; fi; echo "Other/unsupported files cataloged: $HASH_SKIPPED"; echo
  echo "[LIBRARY COMPLETION]"; echo "Regions: $COMPLETION_REGION_LABEL"; if [ "$COMPLETION_RETAIL_ONLY" -eq 1 ]; then echo "Scope: Retail releases only"; else echo "Scope: All release types"; fi; echo "World releases count toward USA, Europe, and Japan when COMPLETION_INCLUDE_WORLD=1."; tail -n +2 "$STAGE_COMPLETION" | while IFS=',' read -r c_sys c_owned c_total c_missing c_pct rest; do c_sys="${c_sys#\"}"; c_sys="${c_sys%\"}"; c_owned="${c_owned#\"}"; c_owned="${c_owned%\"}"; c_total="${c_total#\"}"; c_total="${c_total%\"}"; c_missing="${c_missing#\"}"; c_missing="${c_missing%\"}"; c_pct="${c_pct#\"}"; c_pct="${c_pct%\"}"; echo "$c_sys | owned=$c_owned | reference=$c_total | missing=$c_missing | completion=$c_pct%"; done; echo "Missing-title detail: missing_library_titles.csv"; echo
  echo "[CACHE HEALTH]"; echo "Entries loaded: $CACHE_ENTRIES_LOADED"; echo "Entries reused: $HASH_REUSED"; echo "Entries refreshed: $CACHE_REFRESHED"; echo "Entries not reused/expired: $CACHE_NOT_REUSED"; echo "Cache hit rate: $CACHE_HIT_RATE%"; echo "Database fingerprint: $HASH_DB_FINGERPRINT"; echo
  echo "[PER-SYSTEM PROCESSING]"; for sys in "${!SYSTEM_FILES[@]}"; do echo "$sys | files=${SYSTEM_FILES[$sys]} | dat_eligible=${SYSTEM_ELIGIBLE[$sys]:-0} | matched=${SYSTEM_MATCHED[$sys]:-0} | unmatched=${SYSTEM_UNMATCHED[$sys]:-0} | processing_seconds=${SYSTEM_SECONDS[$sys]:-0}"; done | LC_ALL=C sort; echo
  echo "[TIMING]"; echo "discovery_seconds=$((DISCOVERY_END-DISCOVERY_START))"; echo "save_index_seconds=$((SAVE_END-SAVE_START))"; echo "database_cache_seconds=$((DB_END-DB_START))"; echo "classification_seconds=$((CLASSIFY_END-CLASSIFY_START))"; echo "parallel_full_verify_hash_seconds=$FULL_VERIFY_PARALLEL_SECONDS"; echo "report_processing_seconds=$(( $(date +%s)-REPORT_START ))"; echo "total_seconds_so_far=$(( $(date +%s)-START_TIME ))"; echo
...
PUBLISH_END=$(date +%s); TOTAL_END=$PUBLISH_END
sync

echo; echo "+--------------------------------------------------+"; echo "| AUDIT COMPLETE                                   |"; echo "+--------------------------------------------------+"
echo " Mode              : $AUDIT_MODE"; echo " Games cataloged   : $TOTAL"; echo " DAT matches       : $DAT_MATCHED / $HASHED"; echo " Collisions        : $COLLISIONS"; echo " Save matches      : $SAVE_MATCHES"; echo " Cache hit rate    : $CACHE_HIT_RATE%"; echo " Integrity         : $AUDIT_VERDICT"; echo " Apply             : $APPLY_RECOMMENDATION"
printf '%s\n' '----------------------------------------------------'; echo " Reports: $AUDIT"; echo " Review : MiSTer_Library_Audit.txt"; echo " Missing: missing_library_titles.csv"; printf '%s\n' '----------------------------------------------------'; echo " READ ONLY: no ROMs or saves were changed."; printf '%s\n' '----------------------------------------------------'; echo; echo "Press Enter to close, or wait 60 seconds."; read -t 60 -r _ || true
